...
static circular_buffer buffer2;
...
static inline bool in_spiketimes_initialize_spike_buffer(uint32_t size) {
    buffer2 = circular_buffer_initialize(size);
    return buffer2 != 0;
}

static inline bool in_spiketimes_not_empty() {
    bool empty = _circular_buffer_not_empty(buffer2);
    log_info("spiketime empty = %u", empty);
    return(!empty); 
}
...
static inline bool in_spiketimes_add_spiketime(uint32_t spiketime) {
...
static inline bool in_spiketimes_get_next_spiketime(uint32_t* spiketime) {
    return circular_buffer_get_next(buffer2, spiketime);
}
...
static inline bool in_spiketimes_is_next_spike_equal(spike_t spike) {
    return circular_buffer_advance_if_next_equals(buffer2, spike);
}

//! \brief Get the number of times that the input spike buffer overflowed.
//! \return A count.
static inline counter_t in_spiketimes_get_n_buffer_overflows(void) {
    return circular_buffer_get_n_buffer_overflows(buffer2);
}

//! \brief Get the number of times that the input spike buffer underflowed.
//! \return A count.
static inline counter_t in_spiketimes_get_n_buffer_underflows(void) {
    return 0;
}

//! \brief Print the input spike buffer.
//! \details Expected to be mainly for debugging.
static inline void in_spiketimes_print_buffer(void) {
    circular_buffer_print_buffer(buffer2);
}
...
//!     goes.
//! \return An index.
static inline uint32_t in_spiketimes_input_index(void) {
    return circular_buffer_input(buffer2);
}

//! \brief Get the index in the buffer of the point where the next removal
//!     comes from.
//! \return An index.
static inline uint32_t in_spiketimes_output_index(void) {
    return circular_buffer_output(buffer2);
}

//! \brief Get the size of the input spike buffer.
//! \return The size of the buffer (a power of 2).
static inline uint32_t in_spiketimes_real_size(void) {
    return circular_buffer_real_size(buffer2);
}
...
static inline spike_t in_spiketimes_value_at_index(uint32_t index) {
    return circular_buffer_value_at_index(buffer2, index);
}
#endif // _IN_SPIKES_H_
