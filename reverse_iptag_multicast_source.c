...
#define TICKS_BETWEEN_REQUESTS 500
...
static inline uint32_t extract_time_from_eieio_msg(
    ...
    if (!pkt_apply_prefix && pkt_class) {
        log_info("Packet is command packet");
        return time;
    }
    ...
    for (uint32_t i = 0; i < pkt_count; i++) {
        ...
        if (has_key) {
            if (!check_key_in_space || (key & mask) == key_space) {
                ...
                payload = next_buffer_time;
                log_info("mc packet 32-bit key=%u, payload=%u",
                            key, payload);
...
static void fetch_and_process_packet(void) {
    ...
    while (!msg_from_sdram_in_use && is_eieio_packet_in_buffer() &&
        ...
        if (next_header == 0x4002) {
            ...
            log_info("packet with length %d, from address: %08x", len,
                    read_pointer);
            ...
            log_info("packet time: %d, current time: %d",
...
static void send_buffer_request_pkt(void) {
    ...
    if ((space >= space_before_data_request) &&
        ...
        log_info("sending request packet with space: %d and seq_no: %d at %u",
...
static void timer_callback(uint unused0, uint unused1) {
    ...
    if(time-next_buffer_time == 10){

        log_info("No more spike packets to send");
        simulation_handle_pause_resume(resume_callback);
...
void c_main(void) {
    ...
    timer_period = TICKS_BETWEEN_REQUESTS;
