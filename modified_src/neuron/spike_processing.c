// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/neuron/spike_processing.c
...
static inline void setup_synaptic_dma_write(
    ...
    log_mini_debug("%u%u%08x%u", 8982, write_size,  sdram_start_address,  buffer->originating_spike);
    /* log_debug("Writing back %u bytes of plastic region to %08x for spike %u",write_size, sdram_start_address, buffer->originating_spike);*/
...
static void multicast_packet_received_callback(uint key, uint payload) {
    use(payload);
    log_mini_debug("%u%x%d%d", 8983,  key,  time,  dma_busy);  /* log_debug("Received spike %x at %d, DMA Busy = %d", key, time, dma_busy);*/
    ...
    if (in_spikes_add_spike(key)) {
        ...
        if (!dma_busy) {
            log_mini_debug("%u", 8984);  /* log_debug("Sending user event for new spike");*/
            if (spin1_trigger_user_event(0, 0)) {
                dma_busy = true;
            } else {
                log_mini_debug("%u", 8985);  /* log_debug("Could not trigger user event\n");*/
            }
        }
    } else {
        log_mini_debug("%u", 8986);  /* log_debug("Could not add spike");*/
    }
}
...
static void dma_complete_callback(uint unused, uint tag) {
    ...
    log_mini_debug("%u%u%u", 8987,  time,  tag);  /* log_debug("DMA transfer complete at time %u with tag %u", time, tag);*/
    ...
    while (n_spikes > 0) {
        ...
        if (!synapses_process_synaptic_row(
            ...
            log_mini_error("%u%.8x%.8x%.8x", 8988, current_buffer->originating_spike, current_buffer->sdram_writeback_address, current_buffer->row);
            /* log_error("Error processing spike 0x%.8x for address 0x%.8x (local=0x%.8x)",current_buffer->originating_spike,current_buffer->sdram_writeback_address,current_buffer->row);*/
            ...
            for (uint32_t i = 0;
                ...
                log_mini_error("%u%u%.8x", 8989,  i,  current_buffer->row[i]);  /* log_error("%u: 0x%.8x", i, current_buffer->row[i]);*/
            }
            rt_error(RTE_SWERR);
        }
...
bool spike_processing_initialise( // EXPORTED
    ...
    for (uint32_t i = 0; i < N_DMA_BUFFERS; i++) {
        ...
        if (dma_buffers[i].row == NULL) {
            log_mini_error("%u", 8990);  /* log_error("Could not initialise DMA buffers");*/
            return false;
        }
        log_mini_debug("%u%u%08x", 8991, i,  dma_buffers[i].row);
...
bool spike_processing_do_rewiring(int number_of_rewires) {
    ...
    // flag pipeline as busy and trigger a feed event
    if (!dma_busy) {
        log_mini_debug("%u", 8992);  /* log_debug("Sending user event for rewiring");*/
        if (spin1_trigger_user_event(0, 0)) {
            dma_busy = true;
        } else {
            log_mini_debug("%u", 8993);  /* log_debug("Could not trigger user event\n");*/
        }
    }
    // enable interrupts
    spin1_mode_restore(cpsr);
