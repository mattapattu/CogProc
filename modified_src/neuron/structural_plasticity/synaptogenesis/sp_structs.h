// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/neuron/structural_plasticity/synaptogenesis/sp_structs.h
...
static inline uint8_t *sp_structs_read_in_common(
    ...
    log_mini_debug("%u%u", 9058,  rewiring_data->s_max);  /* log_debug("Topographic Map Impl, s_max=%u", rewiring_data->s_max);*/

    pre_info->no_pre_pops = rewiring_data->no_pre_pops;
    log_mini_debug("%u%d", 9059,  pre_info->no_pre_pops);  /* log_debug(" no pre pops = %d", pre_info->no_pre_pops);*/
    pre_info->prepop_info = spin1_malloc(
            rewiring_data->no_pre_pops * sizeof(pre_info_t *));
    if (pre_info->prepop_info == NULL) {
        log_mini_error("%u", 9060);  /* log_error("Could not initialise pre population info");*/
        rt_error(RTE_SWERR);
    }
    for (uint32_t i = 0; i < rewiring_data->no_pre_pops; i++) {
        ...
        if (pre_info->prepop_info[i] == NULL) {
            log_mini_error("%u%d", 9061,  i);  /* log_error("Could not initialise pre population info %d", i);*/
            rt_error(RTE_SWERR);
        }
        spin1_memcpy(pre_info->prepop_info[i], data, pre_size);

        log_mini_debug("%u%u%u%u%u%d", 9062, pre_info->prepop_info[i]->no_pre_vertices, pre_info->prepop_info[i]->sp_control, pre_info->prepop_info[i]->delay_lo, pre_info->prepop_info[i]->delay_hi, pre_info->prepop_info[i]->weight);
        /* log_debug("no_pre = %u, sp_control %u, delay lo %u, delay hi %u, weight %d",pre_info->prepop_info[i]->no_pre_vertices,pre_info->prepop_info[i]->sp_control,pre_info->prepop_info[i]->delay_lo,pre_info->prepop_info[i]->delay_hi,pre_info->prepop_info[i]->weight);*/





        log_mini_debug("%u%d%d", 9063, pre_info->prepop_info[i]->connection_type, pre_info->prepop_info[i]->total_no_atoms);
        /* log_debug("connection_type = %d, total_no_atoms=%d",pre_info->prepop_info[i]->connection_type,pre_info->prepop_info[i]->total_no_atoms);*/
    ...
    *post_to_pre_table = (post_to_pre_entry *) data;
    ...
    for (uint32_t i=0; i < n_elements; i++){
        log_mini_debug("%u%d%d%d%d", 9064, i,  post_to_pre_table[i]->pop_index, post_to_pre_table[i]->sub_pop_index, post_to_pre_table[i]->neuron_index);
