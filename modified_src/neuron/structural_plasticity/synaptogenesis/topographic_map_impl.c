// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/neuron/structural_plasticity/synaptogenesis/topographic_map_impl.c
...
void print_post_to_pre_entry(void) {
    ...
    for (uint32_t i=0; i < n_elements; i++) {
        log_mini_debug("%u%d%d%d%d", 9065, i,  post_to_pre_table[i].pop_index, post_to_pre_table[i].sub_pop_index, post_to_pre_table[i].neuron_index);
        /* log_debug("index %d, pop index %d, sub pop index %d, neuron_index %d",i, post_to_pre_table[i].pop_index,post_to_pre_table[i].sub_pop_index,post_to_pre_table[i].neuron_index);*/
...
static inline void _queue_state(current_state_t *state) {
    if (__builtin_expect(
        ...
        log_mini_error("%u%08x", 9066,  state);  /* log_error("Could not add state (0x%08x) to queued states", state);*/
        rt_error(RTE_SWERR);
    }
}
...
static inline current_state_t *_get_state(void) {
    ...
    if (__builtin_expect(
            ...
            0)) {
        log_mini_error("%u", 9067);  /* log_error("Could not read a state!");*/
        rt_error(RTE_SWERR);
    }
    return state;
}
...
static inline void _free_state(current_state_t *state) {
    if (__builtin_expect(
        ...
        log_mini_error("%u%08x", 9068,  state);  /* log_error("Could not add state (0x%08x) to free states", state);*/
        rt_error(RTE_SWERR);
    }
}
...
static inline current_state_t *_alloc_state(void) {
    ...
    if (__builtin_expect(
        ...
        log_mini_error("%u", 9069);  /* log_error("Ran out of states!");*/
        rt_error(RTE_SWERR);
    }
    return state;
}
...
bool synaptogenesis_dynamics_initialise(address_t sdram_sp_address) {
    log_mini_debug("%u", 9070);  /* log_debug("SR init.");*/
    ...
    log_mini_debug("%u%u%u%u", 9071, rewiring_data.p_rew,  rewiring_data.fast,  n_states);
    /* log_debug("Rewiring period %u, fast=%u, n_states=%u",rewiring_data.p_rew, rewiring_data.fast, n_states);*/
    ...
    if (current_state_queue == NULL) {
        log_mini_error("%u", 9072);  /* log_error("Could not allocate current state queue");*/
        rt_error(RTE_SWERR);
    }
    // Add one to number of states as buffer wastes an entry
    free_states = circular_buffer_initialize(n_states + 1);
    if (free_states == NULL) {
        log_mini_error("%u", 9073);  /* log_error("Could not allocate free state queue");*/
    }
    current_state_t *states = spin1_malloc(n_states * sizeof(current_state_t));
    if (states == NULL) {
        log_mini_error("%u", 9074);  /* log_error("Could not allocate states");*/
        rt_error(RTE_SWERR);
    }
    ...
    if (formation_params == NULL) {
        log_mini_error("%u", 9075);  /* log_error("Could not initialise formation parameters");*/
        rt_error(RTE_SWERR);
    }
    ...
    if (elimination_params == NULL) {
        log_mini_error("%u", 9076);  /* log_error("Could not initialise elimination parameters");*/
        rt_error(RTE_SWERR);
    }
    for (uint32_t i = 0; i < rewiring_data.no_pre_pops; i++) {
...
bool synaptogenesis_dynamics_rewire(
    ...
    if (!population_table_get_first_address(
        ...
        log_mini_error("%u%d", 9077,  *spike);  /* log_error("FAIL@key %d", *spike);*/
        rt_error(RTE_SWERR);
    }
    uint32_t index = 0;
    while (index < m_pop_index) {
        if (!population_table_get_next_address(
                spike, synaptic_row_address, n_bytes)) {
            log_mini_error("%u%d%d%d", 9078, *spike,  m_pop_index,  index);
            /* log_error("FAIL@key %d, index %d (failed at %d)",*spike, m_pop_index, index);*/
            rt_error(RTE_SWERR);
        }
        index++;
    }
...
bool synaptogenesis_row_restructure(uint32_t time, address_t row) {
    ...
    if (current_state->element_exists) {
        ...
        if (synapse_dynamics_find_neuron(
            ...
            log_mini_debug("%u%d", 9079, current_state->post_syn_id);
        ...
        if (no_elems >= rewiring_data.s_max) {
            log_mini_debug("%u", 9080);  /* log_debug("row is full");*/
            return_value = false;
        } else {
