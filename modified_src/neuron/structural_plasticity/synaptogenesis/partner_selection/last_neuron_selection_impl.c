// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/neuron/structural_plasticity/synaptogenesis/partner_selection/last_neuron_selection_impl.c
...
void partner_init(uint8_t **data) {
    ...
    log_mini_info("%u%u", 9085,  last_spikes_buffer_size);  /* log_info("Last neuron selection, buffer size = %u", last_spikes_buffer_size);*/
    for (uint32_t i = 0; i < 2; i++) {
        last_spikes_buffer[i] =
                spin1_malloc(last_spikes_buffer_size * sizeof(spike_t));
        if (last_spikes_buffer[i] == NULL) {
            log_mini_error("%u", 9086);  /* log_error("Out of memory when creating last spikes buffer");*/
            rt_error(RTE_SWERR);
        }
        n_spikes[i] = 0;
    }
