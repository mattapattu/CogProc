// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/neuron/threshold_types/threshold_type_external_device_control.h
...
static inline uint get_payload(enum send_type type, accum value) {
    ...
    default:
        log_mini_error("%u%u", 9304,  value);  /* log_error("Unknown enum value %u", value);*/
        rt_error(RTE_SWERR);
    }
    return 0;
}
...
static bool threshold_type_is_above_threshold(
    ...
    if (threshold_type->time_until_next_send == 0) {
        if (threshold_type->value_as_payload) {
            ...
            log_mini_debug("%u%08x%08x", 9305, threshold_type->key,  payload);
            /* log_debug("Sending key=0x%08x payload=0x%08x",threshold_type->key, payload);*/
            send_packet(threshold_type->key, payload, true);
        } else {
            log_mini_debug("%u%08x", 9306,  threshold_type->key);  /* log_debug("Sending key=0x%08x", threshold_type->key);*/
