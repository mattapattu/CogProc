// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/neuron/implementations/neuron_impl_standard.h
...
static bool neuron_impl_initialise(uint32_t n_neurons) {
    ...
    if (sizeof(neuron_t)) {
        ...
        if (neuron_array == NULL) {
            log_mini_error("%u", 9141);  /* log_error("Unable to allocate neuron array - Out of DTCM");*/
            return false;
        }
    }
    ...
    if (sizeof(input_type_t)) {
        ...
        if (input_type_array == NULL) {
            log_mini_error("%u", 9142);  /* log_error("Unable to allocate input type array - Out of DTCM");*/
            return false;
        }
    }
    ...
    if (sizeof(additional_input_t)) {
        ...
        if (additional_input_array == NULL) {
            log_mini_error("%u", 9143);
            /* log_error("Unable to allocate additional input array - Out of DTCM");*/
            return false;
        }
    }
    ...
    if (sizeof(threshold_type_t)) {
        ...
        if (threshold_type_array == NULL) {
            log_mini_error("%u", 9144);  /* log_error("Unable to allocate threshold type array - Out of DTCM");*/
            return false;
        }
    }
    ...
    if (sizeof(synapse_param_t)) {
        ...
        if (neuron_synapse_shaping_params == NULL) {
            log_mini_error("%u", 9145);
            /* log_error("Unable to allocate synapse parameters array - Out of DTCM");*/
            return false;
        }
    }
...
static void neuron_impl_load_neuron_parameters(
    ...
    log_mini_debug("%u%u%u", 9146, next,  n_neurons);
    /* log_debug("reading parameters, next is %u, n_neurons is %u ",next, n_neurons);*/

    // Read the number of steps per timestep
    n_steps_per_timestep = address[next++];
    if (n_steps_per_timestep > 1) {
        log_mini_info("%u%u", 9147,  n_steps_per_timestep);  /* log_info("Looping over %u steps each timestep", n_steps_per_timestep);*/
    } else if (n_steps_per_timestep == 0) {
        log_mini_error("%u", 9148);  /* log_error("bad number of steps per timestep: 0");*/
    }

    if (sizeof(global_neuron_params_t)) {
        log_mini_debug("%u", 9149);  /* log_debug("writing neuron global parameters");*/
    ...
    if (sizeof(neuron_t)) {
        log_mini_debug("%u", 9150);  /* log_debug("reading neuron local parameters");*/
    ...
    if (sizeof(input_type_t)) {
        log_mini_debug("%u", 9151);  /* log_debug("reading input type parameters");*/
    ...
    if (sizeof(threshold_type_t)) {
        log_mini_debug("%u", 9152);  /* log_debug("reading threshold type parameters");*/
    ...
    if (sizeof(synapse_param_t)) {
        log_mini_debug("%u", 9153);  /* log_debug("reading synapse parameters");*/
    ...
    if (sizeof(additional_input_t)) {
        log_mini_debug("%u", 9154);  /* log_debug("reading additional input type parameters");*/
    ...
    log_mini_debug("%u", 9155);  /* log_debug("-------------------------------------\n");*/
    for (index_t n = 0; n < n_neurons; n++) {
        neuron_model_print_parameters(&neuron_array[n]);
    }
    log_mini_debug("%u", 9156);  /* log_debug("-------------------------------------\n");*/
...
static bool neuron_impl_do_timestep_update(index_t neuron_index,
 ...
 log_mini_info("%u%12.6k%12.6k", 9157,  total_exc, total_inh);  /* log_info("total_exc: %12.6k, total_inh: %12.6k", total_exc,total_inh);*/
...
static void neuron_impl_store_neuron_parameters(
        address_t address, uint32_t next, uint32_t n_neurons) {
    log_mini_debug("%u", 9158);  /* log_debug("writing parameters");*/

    // Skip over the steps per timestep
    next += 1;

    if (sizeof(global_neuron_params_t)) {
        log_mini_debug("%u", 9159);  /* log_debug("writing neuron global parameters");*/
    ...
    if (sizeof(neuron_t)) {
        log_mini_debug("%u", 9160);  /* log_debug("writing neuron local parameters");*/
    ...
    if (sizeof(input_type_t)) {
        log_mini_debug("%u", 9161);  /* log_debug("writing input type parameters");*/
    ...
    if (sizeof(threshold_type_t)) {
        log_mini_debug("%u", 9162);  /* log_debug("writing threshold type parameters");*/
    ...
    if (sizeof(synapse_param_t)) {
        log_mini_debug("%u", 9163);  /* log_debug("writing synapse parameters");*/
    ...
    if (sizeof(additional_input_t)) {
        log_mini_debug("%u", 9164);  /* log_debug("writing additional input type parameters");*/
...
void neuron_impl_print_inputs(uint32_t n_neurons) {
    ...
    if (!empty) {
        log_mini_debug("%u", 9165);  /* log_debug("-------------------------------------\n");*/
        ...
        for (index_t i = 0; i < n_neurons; i++) {
            ...
            input_t input = synapse_types_get_excitatory_input(params)
                    - synapse_types_get_inhibitory_input(params);
            if (bitsk(input) != 0) {
                log_mini_debug("%u%3u%12.6k", 9166,  i,  input);  /* log_debug("%3u: %12.6k (= ", i, input);*/
                synapse_types_print_input(params);
                log_mini_debug("%u", 9167);  /* log_debug(")\n");*/
            }
        }
        log_mini_debug("%u", 9168);  /* log_debug("-------------------------------------\n");*/
    }
}
...
void neuron_impl_print_synapse_parameters(uint32_t n_neurons) {
    log_mini_debug("%u", 9169);  /* log_debug("-------------------------------------\n");*/
    for (index_t n = 0; n < n_neurons; n++) {
        synapse_types_print_parameters(&neuron_synapse_shaping_params[n]);
    }
    log_mini_debug("%u", 9170);  /* log_debug("-------------------------------------\n");*/
}
