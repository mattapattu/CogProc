...
static inline void print_master_population_table(void) {
    log_mini_info("%u", 9275);  /* log_info("master_population\n");*/
    for (uint32_t i = 0; i < master_population_table_length; i++) {
        ...
        for (uint16_t j = entry.start; j < (entry.start + entry.count); j++) {
            if (!is_single(address_list[j])) {
                log_mini_info("%u%d%d%.8x%.8x%.8x%.8x%u", 9276, i,  j,  entry.key,  entry.mask, get_address(address_list[j]), get_address(address_list[j]) +(uint32_t) synaptic_rows_base_address, get_row_length(address_list[j]));
                /* log_info("index (%d, %d), key: 0x%.8x, mask: 0x%.8x, offset: 0x%.8x, address: 0x%.8x, row_length: %u",i, j, entry.key, entry.mask,get_address(address_list[j]),get_address(address_list[j]) +(uint32_t) synaptic_rows_base_address,get_row_length(address_list[j]));*/
                ...
                log_mini_info("%u%d%d%.8x%.8x%.8x%.8x", 9277, i,  j,  entry.key,  entry.mask, get_direct_address(address_list[j]), get_direct_address(address_list[j]) +direct_rows_base_address);
                /* log_info("index (%d, %d), key: 0x%.8x, mask: 0x%.8x, offset: 0x%.8x, address: 0x%.8x, single",i, j, entry.key, entry.mask,get_direct_address(address_list[j]),get_direct_address(address_list[j]) +direct_rows_base_address);*/
    ...
    log_mini_info("%u%u", 9278,  master_population_table_length);  /* log_info("Population table has %u entries", master_population_table_length);*/
}
//! \}
...
bool population_table_initialise(
    ...
    log_mini_debug("%u", 9279);  /* log_debug("population_table_initialise: starting");*/

    master_population_table_length = table_address[0];
    log_mini_debug("%u%d", 9280,  master_population_table_length);  /* log_debug("master pop table length is %d\n", master_population_table_length);*/
    log_mini_debug("%u%d", 9281, sizeof(master_population_table_entry));
    ...
    log_mini_debug("%u%d", 9282,  n_master_pop_bytes);  /* log_debug("pop table size is %d\n", n_master_pop_bytes);*/
    ...
    if (n_master_pop_bytes != 0) {
        ...
        if (master_population_table == NULL) {
            log_mini_error("%u", 9283);  /* log_error("Could not allocate master population table");*/
            return false;
        }
    }
    ...
    if (n_address_list_bytes != 0) {
        ...
        if (address_list == NULL) {
            log_mini_error("%u", 9284);  /* log_error("Could not allocate master population address list");*/
            return false;
        }
    }

    log_mini_debug("%u%u%u", 9285, master_population_table_length,  n_master_pop_bytes);
    /* log_debug("pop table size: %u (%u bytes)",master_population_table_length, n_master_pop_bytes);*/
    log_mini_debug("%u%u%u", 9286, address_list_length,  n_address_list_bytes);
    /* log_debug("address list size: %u (%u bytes)",address_list_length, n_address_list_bytes);*/
    ...
    log_mini_info("%u%08x", 9287, synapse_rows_address);
    /* log_info("the stored synaptic matrix base address is located at: 0x%08x",synapse_rows_address);*/
    log_mini_info("%u%08x", 9288, direct_rows_address);
    /* log_info("the direct synaptic matrix base address is located at: 0x%08x",direct_rows_address);*/
...
bool population_table_get_first_address(
    ...
    log_mini_debug("%u%d", 9289,  spike);  /* log_debug("searching for key %d", spike);*/
    int position = population_table_position_in_the_master_pop_array(spike);
    log_mini_debug("%u%d", 9290,  position);  /* log_debug("position = %d", position);*/

    // check we don't have a complete miss
    if (position == NOT_IN_MASTER_POP_TABLE_FLAG) {
        invalid_master_pop_hits++;
        log_mini_debug("%u%u", 9291,  ghost_pop_table_searches);  /* log_debug("Ghost searches: %u\n", ghost_pop_table_searches);*/
        log_mini_debug("%u%u%x", 9292, spike,  spike);
        /* log_debug("spike %u (= %x): population not found in master population table",spike, spike);*/
    ...
    if (entry.count == 0) {
        log_mini_debug("%u%u%x", 9293,  spike,  spike);
        /* log_debug("spike %u (= %x): population found in master populationtable but count is 0", spike, spike);*/
    }

    log_mini_debug("%u", 9294);  /* log_debug("about to try to find neuron id");*/
    ...
    log_mini_debug("%u", 9295);  /* log_debug("checking bit field");*/
    if (connectivity_bit_field[position] != NULL) {
        log_mini_debug("%u", 9296);  /* log_debug("can be checked, bitfield is allocated");*/
        ...
        if (!bit_field_test(
            ...
            log_mini_debug("%u", 9297);  /* log_debug("tested and was not set");*/
            bit_field_filtered_packets += 1;
            return false;
        }
        log_mini_debug("%u", 9298);  /* log_debug("was set, carrying on");*/
    } else {
        log_mini_debug("%u", 9299);
        /* log_debug("bit_field was not set up. either its due to a lack of dtcm, or because the bitfield was merged into the routing table");*/

    }

    // going to do a DMA to read the matrix and see if there's a hit.
    log_mini_debug("%u", 9300);  /* log_debug("about to set items");*/
    next_item = entry.start;
    items_to_go = entry.count;
    last_spike = spike;

    log_mini_debug("%u%08x%u%u%u", 9301, spike,  position,  next_item,  items_to_go);
    /* log_debug("spike = %08x, entry_index = %u, start = %u, count = %u",spike, position, next_item, items_to_go);*/
    ...
    // tracks surplus dmas
    if (!get_next) {
        log_mini_debug("%u%d", 9302,  spike);  /* log_debug("found a entry which has a ghost entry for key %d", spike);*/
        ghost_pop_table_searches++;
    }
    return get_next;
}
            ...
            *spike = last_spike;
            ...
            if (row_length > 0) {
                ...
                *n_bytes_to_transfer = stride * sizeof(uint32_t);
                log_mini_debug("%u%u%.8x%u%.8x%u", 9303, last_neuron_id,  block_address,  row_length,  *row_address, *n_bytes_to_transfer);
                /* log_debug("neuron_id = %u, block_address = 0x%.8x,row_length = %u, row_address = 0x%.8x, n_bytes = %u",last_neuron_id, block_address, row_length, *row_address,*n_bytes_to_transfer);*/
