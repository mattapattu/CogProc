// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/neuron/bit_field_filter.h
...
static bool bit_field_filter_initialise(address_t bitfield_region_address) {
    ...
    log_mini_info("%u%d%d%d", 8994,  filter_region->n_filters, filter_region->n_redundancy_filters, filter_region->n_merged_filters);
    /* log_info("Found %d bitfields of which %d have redundancy of which %d merged in", filter_region->n_filters,filter_region->n_redundancy_filters,filter_region->n_merged_filters);*/
    ...
    if (connectivity_bit_field == NULL) {
        log_mini_warning("%u", 8995);
        /* log_warning("couldn't initialise basic bit field holder. Will end up doing possibly more DMA's during the execution than required");*/
    ...
    for (int cur_bit_field = filter_region->n_merged_filters;
        ...
        log_mini_debug("%u%d%d", 8996,  key,  position_in_array);  /* log_debug("putting key %d in position %d", key, position_in_array);*/
        ...
        if (connectivity_bit_field[position_in_array] == NULL) {
            log_mini_debug("%u%d", 8997,  key);
            /* log_debug("could not initialise bit field for key %d, packets with that key will use a DMA to check if the packet targets anything within this core. Potentially slowing down the execution of neurons on this core.", key);*/
            ...
            log_mini_debug("%u%d", 8998,  key);  /* log_debug("bit field for key %d is :", key);*/
            for (uint32_t bit_field_word_index = 0;
                    bit_field_word_index < n_words;
                    bit_field_word_index++){
                log_mini_debug("%u%x", 8999, connectivity_bit_field[position_in_array][bit_field_word_index]);
                /* log_debug("%x",connectivity_bit_field[position_in_array][bit_field_word_index]);*/
