// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/neuron/synapses.c
...
static inline void print_synaptic_row(synaptic_row_t synaptic_row) {
#if LOG_LEVEL >= LOG_DEBUG
    log_mini_debug("%u%08x%u", 9039, (uint32_t) synaptic_row,  synapse_row_plastic_size(synaptic_row));
    ...
    log_mini_debug("%u", 9040);  /* log_debug("----------------------------------------\n");*/
    ...
    log_mini_debug("%u%u%u", 9041, n_fixed_synapses, synapse_row_num_plastic_controls(fixed_region_address));
    ...
    for (uint32_t i = 0; i < n_fixed_synapses; i++) {
        ...
        log_mini_debug("%u%08x%3d%5u", 9042, synapse,  i,  synapse_row_sparse_weight(synapse));
        /* log_debug("%08x [%3d: (w: %5u (=",synapse, i, synapse_row_sparse_weight(synapse));*/
        synapses_print_weight(synapse_row_sparse_weight(synapse),
                ring_buffer_to_input_left_shifts[synapse_type]);
        log_mini_debug("%u%2u%s%3u%08x%08x", 9043, synapse_row_sparse_delay(synapse, synapse_type_index_bits), get_type_char(synapse_type), synapse_row_sparse_index(synapse, synapse_index_mask), SYNAPSE_DELAY_MASK,  synapse_type_index_bits);
        /* log_debug("nA) d: %2u, %s, n = %3u)] - {%08x %08x}\n",synapse_row_sparse_delay(synapse, synapse_type_index_bits),get_type_char(synapse_type),synapse_row_sparse_index(synapse, synapse_index_mask),SYNAPSE_DELAY_MASK, synapse_type_index_bits);*/
    ...
    if (synapse_row_plastic_size(synaptic_row) > 0) {
        log_mini_debug("%u", 9044);  /* log_debug("----------------------------------------\n");*/
    ...
    log_mini_debug("%u", 9045);  /* log_debug("----------------------------------------\n");*/
#else
    use(synaptic_row);
#endif // LOG_LEVEL >= LOG_DEBUG
}
...
static inline void print_ring_buffers(uint32_t time) {
#if LOG_LEVEL >= LOG_DEBUG
    log_mini_debug("%u%u", 9046,  time);  /* log_debug("Ring Buffer at %u\n", time);*/
    log_mini_debug("%u", 9047);  /* log_debug("----------------------------------------\n");*/
    for (uint32_t n = 0; n < n_neurons; n++) {
        ...
        doPrint:
            ...
            log_mini_debug("%u%3d%s", 9048,  n,  get_type_char(t));  /* log_debug("%3d(%s):", n, get_type_char(t));*/
            for (uint32_t d = 0; d < (1 << SYNAPSE_DELAY_BITS); d++) {
                log_mini_debug("%u", 9049);  /* log_debug(" ");*/
            ...
            log_mini_debug("%u", 9050);  /* log_debug("\n");*/
        }
    }
    log_mini_debug("%u", 9051);  /* log_debug("----------------------------------------\n");*/
#else
    use(time);
#endif // LOG_LEVEL >= LOG_DEBUG
}
...
#if LOG_LEVEL >= LOG_DEBUG
    log_mini_debug("%u", 9052);  /* log_debug("Inputs\n");*/
    neuron_print_inputs();
#endif // LOG_LEVEL >= LOG_DEBUG
}
...
bool synapses_initialise(
    ...
    log_mini_debug("%u", 9053);  /* log_debug("synapses_initialise: starting");*/
    ...
    if (ring_buffer_to_input_left_shifts == NULL) {
        log_mini_error("%u", 9054);  /* log_error("Not enough memory to allocate ring buffer");*/
        return false;
    }
    spin1_memcpy(
    ...
    *ring_buffer_to_input_buffer_left_shifts =
    ...
    log_mini_debug("%u", 9055);  /* log_debug("synapses_initialise: completed successfully");*/
    ...
    if (ring_buffers == NULL) {
        log_mini_error("%u%u", 9056, ring_buffer_size);
        /* log_error("Could not allocate %u entries for ring buffers",ring_buffer_size);*/
    }
