// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/neuron/plasticity/stdp/synapse_dynamics_stdp_mad_impl.c
...
static inline final_state_t plasticity_update_synapse(
    ...
    log_mini_debug("%u%u", 9177,  time);  /* log_debug("\tPerforming deferred synapse update at time:%u", time);*/
    log_mini_debug("%u%u%u%u%u%u", 9178, window_begin_time,  window_end_time,  post_window.prev_time, post_window.prev_time_valid,  post_window.num_events);
    /* log_debug("\t\tbegin_time:%u, end_time:%u - prev_time:%u (valid %u), num_events:%u",window_begin_time, window_end_time, post_window.prev_time,post_window.prev_time_valid, post_window.num_events);*/
    ...
    while (post_window.num_events > 0) {
        ...
        log_mini_debug("%u%u%u", 9179, delayed_post_time,  delayed_last_pre_time);
    ...
    if (post_window.prev_time_valid) {
        ...
        log_mini_debug("%u%u%u", 9180, delayed_pre_time,  delayed_last_post);
...
void synapse_dynamics_print_plastic_synapses(
    ...
    log_mini_debug("%u%u", 9181,  plastic_synapse);  /* log_debug("Plastic region %u synapses\n", plastic_synapse);*/
    ...
    for (uint32_t i = 0; i < plastic_synapse; i++) {
        ...
        log_mini_debug("%u%08x%3d%5u", 9182,  control_word,  i,  weight);  /* log_debug("%08x [%3d: (w: %5u (=", control_word, i, weight);*/
        synapses_print_weight(
                weight, ring_buffer_to_input_buffer_left_shifts[synapse_type]);
        log_mini_debug("%u%2u%s%3u%08x%08x", 9183, synapse_row_sparse_delay(control_word, synapse_type_index_bits), synapse_types_get_type_char(synapse_type), synapse_row_sparse_index(control_word, synapse_index_mask), SYNAPSE_DELAY_MASK,  synapse_type_index_bits);
...
bool synapse_dynamics_process_plastic_synapses(
    ...
    log_mini_debug("%u%u", 9184,  time);  /* log_debug("Adding pre-synaptic event to trace at time:%u", time);*/
...
void synapse_dynamics_process_post_synaptic_event(
    ...
    log_mini_debug("%u%u", 9185,  time);  /* log_debug("Adding post-synaptic event to trace at time:%u", time);*/
