// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/neuron/plasticity/stdp/timing_dependence/timing_recurrent_common.h
...
static inline post_trace_t timing_add_post_spike(
    ...
    log_mini_debug("%u%u", 9237,  time - last_time);  /* log_debug("\tdelta_time=%u", time - last_time);*/
...
static inline pre_trace_t timing_add_pre_spike(
    ...
    log_mini_debug("%u%u", 9238,  time - last_time);  /* log_debug("\tdelta_time=%u", time - last_time);*/
...
static inline update_state_t timing_apply_pre_spike(
    ...
    case STATE_IDLE:
        ...
        log_mini_debug("%u", 9239);  /* log_debug("\tOpening pre-window");*/
    ...
    case STATE_PRE_OPEN:
        ...
        log_mini_debug("%u%u", 9240,  time_since_last_pre);  /* log_debug("\tTime_since_last_pre_event=%u", time_since_last_pre);*/

        if (timing_recurrent_in_pre_window(time_since_last_pre, previous_state)) {
            // If pre-window is still open
            log_mini_debug("%u", 9241);  /* log_debug("\t\tClosing pre-window");*/
            previous_state.state = STATE_IDLE;
        } else {
            // Otherwise, leave state alone (essentially re-opening window)
            log_mini_debug("%u", 9242);  /* log_debug("\t\tRe-opening pre-window");*/
            previous_state =
                    timing_recurrent_calculate_pre_window(previous_state);
        }
        break;
    case STATE_POST_OPEN:
        ...
        log_mini_debug("%u%u", 9243,  time_since_last_post);  /* log_debug("\tTime_since_last_post_event=%u", time_since_last_post);*/
        ...
        if (timing_recurrent_in_post_window(
            ...
            if (previous_state.accumulator >
                ...
                log_mini_debug("%u%d", 9244, previous_state.accumulator);
                /* log_debug("\t\tDecrementing accumulator=%d",previous_state.accumulator);*/
            } else {
                // Otherwise, reset accumulator and apply depression
                log_mini_debug("%u", 9245);  /* log_debug("\t\tApplying depression");*/
            ...
            log_mini_debug("%u", 9246);  /* log_debug("\t\tPost-window closed - Opening pre-window");*/
            previous_state.state = STATE_PRE_OPEN;
            previous_state =
                    timing_recurrent_calculate_pre_window(previous_state);
        }
        break;
    default:
        log_mini_debug("%u%u", 9247,  previous_state.state);  /* log_debug("\tInvalid state %u", previous_state.state);*/
    }
...
static inline update_state_t timing_apply_post_spike(
    ...
    case STATE_IDLE:
        ...
        log_mini_debug("%u", 9248);  /* log_debug("\tOpening post-window");*/
    ...
    case STATE_POST_OPEN:
        ...
        log_mini_debug("%u%u", 9249,  time_since_last_post);  /* log_debug("\tTime_since_last_post_event=%u", time_since_last_post);*/

        if (timing_recurrent_in_post_window(
                time_since_last_post, previous_state)) {
            // If post window's still open
            log_mini_debug("%u", 9250);  /* log_debug("\t\tClosing post-window");*/
            previous_state.state = STATE_IDLE;
        } else {
            // Otherwise, leave state alone (essentially re-opening window)
            log_mini_debug("%u", 9251);  /* log_debug("\t\tRe-opening post-window");*/
            previous_state =
                    timing_recurrent_calculate_post_window(previous_state);
        }
        break;
    case STATE_PRE_OPEN:
        ...
        log_mini_debug("%u%u", 9252,  time_since_last_pre);  /* log_debug("\tTime_since_last_pre_event=%u", time_since_last_pre);*/

        if (time_since_last_pre == 0) {
            // If post-synaptic spike occurred at the same time, ignore it
            log_mini_debug("%u", 9253);  /* log_debug("\t\tIgnoring coinciding spikes");*/
            ...
            if (previous_state.accumulator <
                ...
                // increment it
                previous_state.accumulator++;
                log_mini_debug("%u%d", 9254, previous_state.accumulator);
                /* log_debug("\t\tIncrementing accumulator=%d",previous_state.accumulator);*/
            } else {
                // Otherwise, reset accumulator and apply potentiation
                log_mini_debug("%u", 9255);  /* log_debug("\t\tApplying potentiation");*/
            ...
            // Otherwise, if post-window has closed, skip idle state and go
            // straight to pre-open
            log_mini_debug("%u", 9256);  /* log_debug("\t\tPre-window closed - Opening post-window");*/
            previous_state.state = STATE_POST_OPEN;
            previous_state =
                    timing_recurrent_calculate_post_window(previous_state);
        }
        break;
    default:
        log_mini_debug("%u%u", 9257,  previous_state.state);  /* log_debug("\tInvalid state %u", previous_state.state);*/
    }
