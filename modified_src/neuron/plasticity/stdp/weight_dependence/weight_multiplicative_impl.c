...
address_t weight_initialise(
    ...
    log_mini_debug("%u", 9199);  /* log_debug("weight_initialise: starting");*/
    log_mini_debug("%u", 9200);  /* log_debug("\tSTDP multiplicative weight dependence");*/
    ...
    if (plasticity_weight_region_data == NULL) {
        log_mini_error("%u", 9201);  /* log_error("Could not initialise weight region data");*/
        return NULL;
    }
    weight_multiply_right_shift =
            spin1_malloc(sizeof(uint32_t) * n_synapse_types);
    if (weight_multiply_right_shift == NULL) {
        log_mini_error("%u", 9202);  /* log_error("Could not initialise weight region data");*/
        return NULL;
    }
    ...
    for (uint32_t s = 0; s < n_synapse_types; s++) {
        ...
        log_mini_debug("%u%u%d%d%d%d%u", 9203, s,  plasticity_weight_region_data[s].min_weight, plasticity_weight_region_data[s].max_weight, plasticity_weight_region_data[s].a2_plus, plasticity_weight_region_data[s].a2_minus, weight_multiply_right_shift[s]);
        /* log_debug("\tSynapse type %u: Min weight:%d, Max weight:%d, A2+:%d, A2-:%d, Weight multiply right shift:%u",s, plasticity_weight_region_data[s].min_weight,plasticity_weight_region_data[s].max_weight,plasticity_weight_region_data[s].a2_plus,plasticity_weight_region_data[s].a2_minus,weight_multiply_right_shift[s]);*/
    ...
    log_mini_debug("%u", 9204);  /* log_debug("weight_initialise: completed successfully");*/
