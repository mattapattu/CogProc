...
address_t weight_initialise(
    ...
    log_mini_debug("%u", 9194);  /* log_debug("weight_initialise: starting");*/
    log_mini_debug("%u", 9195);  /* log_debug("\tSTDP additive one-term weight dependence");*/
    ...
    if (plasticity_weight_region_data == NULL) {
        log_mini_error("%u", 9196);  /* log_error("Could not initialise weight region data");*/
        return NULL;
    }
    for (uint32_t s = 0; s < n_synapse_types; s++) {
        ...
        log_mini_debug("%u%u%d%d%d%d", 9197, s,  plasticity_weight_region_data[s].min_weight, plasticity_weight_region_data[s].max_weight, plasticity_weight_region_data[s].a2_plus, plasticity_weight_region_data[s].a2_minus);
        /* log_debug("\tSynapse type %u: Min weight:%d, Max weight:%d, A2+:%d, A2-:%d",s, plasticity_weight_region_data[s].min_weight,plasticity_weight_region_data[s].max_weight,plasticity_weight_region_data[s].a2_plus,plasticity_weight_region_data[s].a2_minus);*/



    }
    log_mini_debug("%u", 9198);  /* log_debug("weight_initialise: completed successfully");*/
