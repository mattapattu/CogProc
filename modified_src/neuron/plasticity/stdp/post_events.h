// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/neuron/plasticity/stdp/post_events.h
...
static inline void print_event_history(const post_event_history_t *events) {
    log_mini_info("%u", 9172);  /* log_info("      ##  printing entire post event history  ##");*/
    for (uint32_t i = 0; i <= events->count_minus_one; i++) {
        log_mini_info("%u%u%u%u", 9173, i,  events->times[i],  events->traces[i]);
        /* log_info("post event: %u, time: %u, trace: %u",i, events->times[i], events->traces[i]);*/
    }
}
...
static inline void print_delayed_window_events(
    ...
    log_mini_debug("%u", 9175);  /* log_debug("     ##  printing post window  ##");*/
    ...
    while (post_window.num_events > 0) {
        ...
        log_mini_debug("%u%u%u%u", 9176, post_window.num_events,  delayed_post_time, *post_window.next_trace);
        /* log_debug("post spike: %u, time: %u, trace: %u",post_window.num_events, delayed_post_time,*post_window.next_trace);*/
