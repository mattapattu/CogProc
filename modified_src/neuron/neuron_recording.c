// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/neuron/neuron_recording.c
...
static bool neuron_recording_read_in_elements(
    ...
    for (uint32_t i = 0; i < N_RECORDED_VARS; i++) {
        ...
        if (recording_info[i].values == NULL) {
            ...
            if (recording_info[i].values == NULL) {
                log_mini_error("%u%d", 9010,  i);  /* log_error("couldn't allocate recording data space for %d", i);*/
                return false;
            }
    ...
    for (uint32_t i = 0; i < N_BITFIELD_VARS; i++) {
        ...
        if (bitfield_info[i].values == NULL) {
            ...
            if (bitfield_info[i].values == NULL) {
                log_mini_error("%u%d", 9011,  i);  /* log_error("couldn't allocate bitfield recording data space for %d", i);*/
                return false;
            }
...
bool neuron_recording_reset(uint32_t n_neurons) {
    ...
    if (!neuron_recording_read_in_elements(reset_address, n_neurons)) {
        log_mini_error("%u", 9012);  /* log_error("failed to reread in the new elements after reset");*/
        return false;
    }
    return true;
}
...
static inline bool allocate_word_dtcm(uint32_t n_neurons) {
    ...
    if (recording_info == NULL) {
        log_mini_error("%u", 9013);  /* log_error("Could not allocated space for recording_info");*/
        return false;
    }
    ...
    if (neuron_recording_indexes == NULL) {
        log_mini_error("%u", 9014);  /* log_error("Could not allocate space for var_recording_indexes");*/
        return false;
    }

    recording_values = spin1_malloc(N_RECORDED_VARS * sizeof(uint8_t *));
    if (recording_values == NULL) {
        log_mini_error("%u", 9015);  /* log_error("Could not allocate space for recording_values");*/
        return false;
    }
    ...
    for (uint32_t i = 0; i < N_RECORDED_VARS; i++) {
        ...
        if (neuron_recording_indexes[i] == NULL) {
            log_mini_error("%u%d", 9016,  i);  /* log_error("failed to allocate memory for recording index %d", i);*/
            return false;
        }
    }
...
static inline bool allocate_bitfield_dtcm(uint32_t n_neurons) {
    ...
    if (bitfield_info == NULL) {
        log_mini_error("%u", 9017);  /* log_error("Failed to allocate space for bitfield_info");*/
        return false;
    }
    ...
    if (bitfield_recording_indexes == NULL) {
        log_mini_error("%u", 9018);  /* log_error("Could not allocate space for bitfield_recording_indexes");*/
        return false;
    }

    bitfield_values = spin1_malloc(N_BITFIELD_VARS * sizeof(uint32_t *));
    if (bitfield_values == NULL) {
        log_mini_error("%u", 9019);  /* log_error("Could not allocate space for bitfield_values");*/
        return false;
    }
    ...
    for (uint32_t i = 0; i < N_BITFIELD_VARS; i++) {
        ...
        if (bitfield_recording_indexes[i] == NULL) {
            log_mini_error("%u%d", 9020,  i);  /* log_error("failed to allocate memory for bitfield index %d", i);*/
            return false;
        }
    }
...
bool neuron_recording_initialise(
    ...
    if (!success) {
        log_mini_error("%u", 9021);  /* log_error("failed to init basic recording.");*/
        return false;
    }
    ...
    if (header->n_recorded_vars != N_RECORDED_VARS) {
        log_mini_error("%u%d%d", 9022, header->n_recorded_vars,  N_RECORDED_VARS);
        /* log_error("Data spec number of recording variables %d != neuron implementation number of recorded variables %d",header->n_recorded_vars, N_RECORDED_VARS);*/

        return false;
    }
    if (header->n_bitfield_vars != N_BITFIELD_VARS) {
        log_mini_error("%u%d%d", 9023, header->n_bitfield_vars,  N_BITFIELD_VARS);
        /* log_error("Data spec number of bitfield variables %d != neuron implementation number of bitfield variables %d",header->n_bitfield_vars, N_BITFIELD_VARS);*/

        return false;
    }
    data_addr = &header[1];
    log_mini_info("%u%d%d", 9024, N_RECORDED_VARS,  N_BITFIELD_VARS);
    /* log_info("Recording %d variables and %d bitfield variables",N_RECORDED_VARS, N_BITFIELD_VARS);*/

    if (!allocate_word_dtcm(n_neurons)) {
        log_mini_error("%u", 9025);  /* log_error("failed to allocate DTCM for the neuron recording structs.");*/
        return false;
    }
    if (!allocate_bitfield_dtcm(n_neurons)) {
        log_mini_error("%u", 9026);  /* log_error("failed to allocate DTCM for the bitfield recording structs");*/
        return false;
    }
    ...
    if (!neuron_recording_read_in_elements(data_addr, n_neurons)) {
        log_mini_error("%u", 9027);  /* log_error("failed to read in the elements");*/
        return false;
    }
