// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/neuron/neuron.c
...
static bool neuron_load_neuron_parameters(address_t address) {
    log_mini_debug("%u", 9028);  /* log_debug("loading parameters");*/
...
bool neuron_resume(address_t address) { // EXPORTED
    if (!neuron_recording_reset(n_neurons)){
        log_mini_error("%u", 9029);  /* log_error("failed to reload the neuron recording parameters");*/
        return false;
    }

    log_mini_debug("%u", 9030);  /* log_debug("neuron_reloading_neuron_parameters: starting");*/
...
bool neuron_initialise(address_t address, address_t recording_address, // EXPORTED
    ...
    log_mini_debug("%u", 9031);  /* log_debug("neuron_initialise: starting");*/
    ...
    *timer_offset = params->timer_start_offset;
    ...
    log_mini_debug("%u%u%u", 9032, *timer_offset,  time_between_spikes);
    /* log_debug("\t back off = %u, time between spikes %u",*timer_offset, time_between_spikes);*/
    ...
    if (!use_key) {
        log_mini_debug("%u", 9033);  /* log_debug("\tThis model is not expecting to transmit as it has no key");*/
    } else {
        log_mini_debug("%u%08x", 9034,  key);  /* log_debug("\tThis model is expected to transmit with key = %08x", key);*/
    }
    ...
    *incoming_spike_buffer_size = params->incoming_spike_buffer_size;
    ...
    log_mini_debug("%u%u%u", 9035,  n_neurons, *incoming_spike_buffer_size);
    /* log_debug("\t n_neurons = %u, spike buffer size = %u", n_neurons,*incoming_spike_buffer_size);*/
...
void neuron_pause(address_t address) { // EXPORTED
    ...
    if (recording_flags > 0) {
        log_mini_debug("%u", 9036);  /* log_debug("updating recording regions");*/
...
void neuron_do_timestep_update( // EXPORTED
    ...
    for (index_t neuron_index = 0; neuron_index < n_neurons; neuron_index++) {
        ...
        if (spike) {
            log_mini_debug("%u%u%u", 9037,  neuron_index,  time);  /* log_debug("neuron %u spiked at time %u", neuron_index, time);*/
            ...
            log_mini_debug("%u%d", 9038, neuron_index);
            /* log_debug("the neuron %d has been determined to not spike",neuron_index);*/
         }
    }
