// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/neuron/direct_synapses.c
...
bool direct_synapses_initialise(
    ...
    log_mini_info("%u%d", 8979,  direct_matrix_size);  /* log_info("Direct matrix malloc size is %d", direct_matrix_size);*/

    if (direct_matrix_size != 0) {
        *direct_synapses_address = spin1_malloc(direct_matrix_size);
        if (*direct_synapses_address == NULL) {
            log_mini_error("%u", 8980);  /* log_error("Not enough memory to allocate direct matrix");*/
            return false;
        }
        log_mini_debug("%u%u%08x", 8981, direct_matrix_size,  *direct_synapses_address);
        /* log_debug("Copying %u bytes of direct synapses to 0x%08x",direct_matrix_size, *direct_synapses_address);*/
