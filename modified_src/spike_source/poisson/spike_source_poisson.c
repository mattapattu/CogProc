// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/spike_source/poisson/spike_source_poisson.c
...
static void print_spike_source(index_t s) {
    ...
    log_mini_info("%u%d", 9418,  s);  /* log_info("atom %d", s);*/
    log_mini_info("%u%u", 9419,  p->start_ticks);  /* log_info("scaled_start = %u", p->start_ticks);*/
    log_mini_info("%u%u", 9420,  p->end_ticks);  /* log_info("scaled end = %u", p->end_ticks);*/
    log_mini_info("%u%u", 9421,  p->next_ticks);  /* log_info("scaled next = %u", p->next_ticks);*/
    log_mini_info("%u%d", 9422,  p->is_fast_source);  /* log_info("is_fast_source = %d", p->is_fast_source);*/
    log_mini_info("%u%k", 9423,  (REAL) p->exp_minus_lambda);  /* log_info("exp_minus_lamda = %k", (REAL) p->exp_minus_lambda);*/
    log_mini_info("%u%k", 9424,  p->mean_isi_ticks);  /* log_info("isi_val = %k", p->mean_isi_ticks);*/
    log_mini_info("%u%k", 9425,  p->time_to_spike_ticks);  /* log_info("time_to_spike = %k", p->time_to_spike_ticks);*/
}
...
static bool read_global_parameters(global_parameters *sdram_globals) {
    log_mini_info("%u", 9426);  /* log_info("read global_parameters: starting");*/

    spin1_memcpy(&ssp_params, sdram_globals, sizeof(ssp_params));

    log_mini_info("%u%08x%08x%u", 9427, ssp_params.key,  ssp_params.set_rate_neuron_id_mask, ssp_params.timer_offset);
    /* log_info("\tkey = %08x, set rate mask = %08x, timer offset = %u",ssp_params.key, ssp_params.set_rate_neuron_id_mask,ssp_params.timer_offset);*/

    log_mini_info("%u%u%u%u%u", 9428,  ssp_params.spike_source_seed[0], ssp_params.spike_source_seed[1], ssp_params.spike_source_seed[2], ssp_params.spike_source_seed[3]);
    /* log_info("\tseed = %u %u %u %u", ssp_params.spike_source_seed[0],ssp_params.spike_source_seed[1],ssp_params.spike_source_seed[2],ssp_params.spike_source_seed[3]);*/



    log_mini_info("%u%u%u", 9429, ssp_params.n_spike_sources,  ssp_params.first_source_id);
    /* log_info("\tspike sources = %u, starting at %u",ssp_params.n_spike_sources, ssp_params.first_source_id);*/
    log_mini_info("%u%k", 9430,  (REAL) ssp_params.seconds_per_tick);  /* log_info("seconds_per_tick = %k\n", (REAL) ssp_params.seconds_per_tick);*/
    log_mini_info("%u%k", 9431,  ssp_params.ticks_per_second);  /* log_info("ticks_per_second = %k\n", ssp_params.ticks_per_second);*/
    log_mini_info("%u%k", 9432, ssp_params.slow_rate_per_tick_cutoff);
    /* log_info("slow_rate_per_tick_cutoff = %k\n",ssp_params.slow_rate_per_tick_cutoff);*/
    log_mini_info("%u%k", 9433, ssp_params.fast_rate_per_tick_cutoff);
    /* log_info("fast_rate_per_tick_cutoff = %k\n",ssp_params.fast_rate_per_tick_cutoff);*/

    log_mini_info("%u", 9434);  /* log_info("read_global_parameters: completed successfully");*/
    return true;
}
...
static bool read_rates(source_info *sdram_sources) {
    ...
    if (ssp_params.n_spike_sources > 0) {
        ...
        if (source == NULL) {
            ...
            if (source == NULL) {
                log_mini_error("%u", 9435);  /* log_error("Failed to allocate local sources");*/
                return false;
            }
            source_data = spin1_malloc(
                    ssp_params.n_spike_sources * sizeof(source_info *));
            if (source_data == NULL) {
                log_mini_error("%u", 9436);  /* log_error("Failed to allocate SDRAM source links");*/
                return false;
            }
    ...
    log_mini_info("%u", 9437);  /* log_info("read_poisson_parameters: completed successfully");*/
    return true;
}
...
static bool initialise_recording(data_specification_metadata_t *ds_regions) {
    ...
    log_mini_info("%u%08x", 9438,  recording_flags);  /* log_info("Recording flags = 0x%08x", recording_flags);*/
...
static bool initialize(void) {
    log_mini_info("%u", 9439);  /* log_info("Initialise: started");*/
    ...
    log_mini_info("%u", 9440);  /* log_info("Initialise: completed successfully");*/
...
static void resume_callback(void) {
    ...
    if (!read_global_parameters(
            data_specification_get_region(POISSON_PARAMS, ds_regions))) {
        log_mini_error("%u", 9441);  /* log_error("failed to reread the Poisson parameters from SDRAM");*/
        rt_error(RTE_SWERR);
    }

    if (!read_rates(
            data_specification_get_region(RATES, ds_regions))){
        log_mini_error("%u", 9442);  /* log_error("failed to reread the Poisson rates from SDRAM");*/
        rt_error(RTE_SWERR);
    }
    ...
    log_mini_info("%u%u", 9443,  time);  /* log_info("Successfully resumed Poisson spike source at time: %u", time);*/
...
static bool store_poisson_parameters(void) {
    log_mini_info("%u", 9444);  /* log_info("store_parameters: starting");*/
    ...
    log_mini_info("%u", 9445);  /* log_info("store_parameters: completed successfully");*/
    return true;
}
...
static void send_spike(uint32_t spike_key, uint32_t timer_count) {
    ...
    // Send the spike
    log_mini_debug("%u%x%d", 9446,  spike_key,  time);  /* log_debug("Sending spike packet %x at %d\n", spike_key, time);*/
...
static inline void expand_spike_recording_buffer(uint32_t n_spikes) {
    ...
    if (new_spikes == NULL) {
        log_mini_error("%u", 9447);  /* log_error("Cannot reallocate spike buffer");*/
        rt_error(RTE_SWERR);
    }
...
static void process_fast_source(
    ...
    if ((time >= source->start_ticks) && (time < source->end_ticks)) {
        ...
        log_mini_debug("%u%d", 9448,  num_spikes);  /* log_debug("Generating %d spikes", num_spikes);*/
...
static void timer_callback(uint timer_count, uint unused) {
    ...
    log_mini_debug("%u%u", 9449,  time);  /* log_debug("Timer tick %u", time);*/
    ...
    if (infinite_run != TRUE && time >= simulation_ticks) {
        ...
        if (!store_poisson_parameters()) {
            log_mini_error("%u", 9450);  /* log_error("Failed to write poisson parameters to SDRAM");*/
            rt_error(RTE_SWERR);
        }
    ...
    for (index_t s_id = 0; s_id < ssp_params.n_spike_sources; s_id++) {
        ...
        if ((time + 1) >= spike_source->next_ticks) {
            log_mini_debug("%u%d", 9451,  time);  /* log_debug("Moving to next rate at time %d", time);*/
...
void set_spike_source_rate(uint32_t id, REAL rate) {
    ...
    log_mini_debug("%u%u%u%k%k", 9452, id,  sub_id,  rate,  rate_per_tick);
    /* log_debug("Setting rate of %u (%u) to %kHz (%k per tick)",id, sub_id, rate, rate_per_tick);*/
...
void c_main(void) {
    // Load DTCM data
    if (!initialize()) {
        log_mini_error("%u", 9453);  /* log_error("Error in initialisation - exiting!");*/
        rt_error(RTE_SWERR);
    }
