// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/common/munich_protocol.h
...
munich_protocol_push_bot_laser_config_total_period(
    ...
    if (mode != MUNICH_PROTOCOL_PUSH_BOT) {
        log_mini_error("%u%d", 9454,  mode);
        /* log_error("The mode you configured is not the pushbot, and so this message is invalid for mode %d", mode);*/
    }
...
static inline multicast_packet _key_retina(
    ...
    log_mini_error("%u", 9469);  /* log_error("The number of pixels is not supported in this protocol.");*/
    rt_error(RTE_SWERR);
...
static inline multicast_packet munich_protocol_set_retina_transmission(
    ...
    if (events_in_key) {
        ...
        default:
            log_mini_error("%u%d", 9470, size_of_time_stamp_in_bytes);
        ...
        log_mini_warning("%u", 9471);
        /* log_warning("The current sPyNNaker models do not support the reception of packets with payloads, therefore you will need to add a adaptor model between the device and sPyNNaker models.");*/


        // verify that its what the end user wants.
        if (payload_holds_time_stamps || (size_of_time_stamp_in_bytes == 0)) {
            log_mini_error("%u", 9472);
            /* log_error("If you are using payloads to store events, you cannot have time stamps at all.");*/
