// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/bit_field_expander/bit_field_expander.c
...
void read_in_addresses(void) {
    ...
    log_mini_info("%u%d", 9473, builder_data->structural_matrix_region_id);
    /* log_info("structural matrix region id = %d",builder_data->structural_matrix_region_id);*/
    ...
    // printer
    log_mini_debug("%u%0x", 9474,  master_pop_base_address);  /* log_debug("master_pop_table_base_address = %0x", master_pop_base_address);*/
    log_mini_debug("%u%0x", 9475, synaptic_matrix_base_address);
    /* log_debug("synaptic_matrix_base_address = %0x",synaptic_matrix_base_address);*/
    log_mini_debug("%u%0x", 9476,  bit_field_base_address);  /* log_debug("bit_field_base_address = %0x", bit_field_base_address);*/
    log_mini_debug("%u%0x", 9477, direct_matrix_region_base_address);
    /* log_debug("direct_matrix_region_base_address = %0x",direct_matrix_region_base_address);*/
    log_mini_info("%u%0x", 9478, structural_matrix_region_base_address);
    /* log_info("structural matrix region base address = %0x",structural_matrix_region_base_address);*/
    log_mini_info("%u", 9479);  /* log_info("finished reading in vertex data region addresses");*/
}

#ifdef __PRINT_KEY_ATOM_MAP__
//! Debugging: print the map
static void print_key_to_max_atom_map(void) {
    log_mini_info("%u%d", 9480,  keys_to_max_atoms->n_pairs);  /* log_info("n items is %d", keys_to_max_atoms->n_pairs);*/
    ...
    for (int key_to_max_atom_index = 0;
        ...
        // print
        log_mini_info("%u%d%x%d", 9481, key_to_max_atom_index, keys_to_max_atoms->pairs[key_to_max_atom_index].key, keys_to_max_atoms->pairs[key_to_max_atom_index].n_atoms);
        /* log_info("entry %d has key %x and n_atoms of %d",key_to_max_atom_index,keys_to_max_atoms->pairs[key_to_max_atom_index].key,keys_to_max_atoms->pairs[key_to_max_atom_index].n_atoms);*/
...
static uint32_t n_neurons_from_key(uint32_t key) {
    int key_index = 0;
    log_mini_debug("%u%d", 9482,  keys_to_max_atoms->n_pairs);  /* log_debug("n pairs is %d", keys_to_max_atoms->n_pairs);*/
    ...
    log_mini_error("%u%x", 9483,  key);  /* log_error("did not find the key %x in the map. WTF!", key);*/
    log_mini_error("%u%d", 9484,  keys_to_max_atoms->n_pairs);  /* log_error("n pairs is %d", keys_to_max_atoms->n_pairs);*/
    for (int pair_id = 0; pair_id < keys_to_max_atoms->n_pairs; pair_id++) {
        key_atom_pair_t entry = keys_to_max_atoms->pairs[key_index];
        log_mini_error("%u%d%x%d", 9485, pair_id,  entry.key,  entry.key == key);
        /* log_error("key at index %d is %x and equal = %d",pair_id, entry.key, entry.key == key);*/
    }
    rt_error(RTE_SWERR);
    return NULL;
}
...
static bool create_fake_bit_field(void) {
    ...
    if (fake_bit_fields == NULL) {
        log_mini_error("%u", 9486);  /* log_error("failed to alloc dtcm for the fake bitfield holders");*/
        return false;
    }
    ...
    log_mini_info("%u", 9487);  /* log_info("finished fake bit field");*/
    return true;
}
...
bool initialise(void) {
    ...
    log_mini_info("%u", 9488);  /* log_info("direct synapse init");*/
    if (!direct_synapses_initialise(
            direct_matrix_region_base_address, &direct_synapses_address)) {
        log_mini_error("%u", 9489);  /* log_error("failed to init the synapses. failing");*/
        return false;
    }

    // init the master pop table
    log_mini_info("%u", 9490);  /* log_info("pop table init");*/
    if (!population_table_initialise(
            master_pop_base_address, synaptic_matrix_base_address,
            direct_synapses_address, &row_max_n_words)) {
        log_mini_error("%u", 9491);  /* log_error("failed to init the master pop table. failing");*/
        return false;
    }

    log_mini_info("%u", 9492);  /* log_info("structural plastic if needed");*/
    if (structural_matrix_region_base_address != NULL) {
        if (! sp_structs_read_in_common(
            ...
            log_mini_error("%u", 9493);  /* log_error("failed to init the synaptogenesis");*/
            return false;
        }
    }

    log_mini_info("%u%d%d", 9494, population_table_length(),  row_max_n_words);
    /* log_info(" elements in master pop table is %d \n and max rows is %d",population_table_length(), row_max_n_words);*/

    if (population_table_length() == 0) {
         success_shut_down();
         log_mini_info("%u", 9495);  /* log_info("successfully processed the bitfields as there wasn't any");*/
         can_run = false;
         return true;
    }
    ...
    if (!create_fake_bit_field()) {
        log_mini_error("%u", 9496);  /* log_error("failed to create fake bit field");*/
        return false;
    }

    // set up a sdram read for a row
    log_mini_info("%u", 9497);  /* log_info("allocating dtcm for row data");*/
    row_data = spin1_malloc(row_max_n_words * sizeof(uint32_t));
    if (row_data == NULL) {
        log_mini_error("%u", 9498);  /* log_error("could not allocate dtcm for the row data");*/
        return false;
    }
    log_mini_debug("%u", 9499);  /* log_debug("finished dtcm for row data");*/
    // set up the fake connectivity lookup into the master pop table

    population_table_set_connectivity_bit_field(fake_bit_fields);
    log_mini_info("%u", 9500);  /* log_info("finished pop table set connectivity lookup");*/
...
bool process_synaptic_row(synaptic_row_t row) {
    ...
    if (synapse_row_plastic_size(row) > 0) {
        log_mini_debug("%u", 9501);  /* log_debug("plastic row had entries, so cant be pruned");*/
        return true;
    }
    ...
    if (fixed_synapse == 0) {
        log_mini_debug("%u", 9502);  /* log_debug("plastic and fixed do not have entries, so can be pruned");*/
        return false;
    } else {
        log_mini_debug("%u", 9503);  /* log_debug("fixed row has entries, so cant be pruned");*/
        return true;
    }
}
...
static bool do_sdram_read_and_test(
    ...
    log_mini_debug("%u", 9504);  /* log_debug("process synaptic row");*/
...
bool generate_bit_field(void) {
    ...
    log_mini_debug("%u", 9505);  /* log_debug("update by pop length");*/
    ...
    log_mini_info("%u%x", 9506,  bit_field_words_location);  /* log_info("words location is %x", bit_field_words_location);*/
    int position = 0;

     // iterate through the master pop entries
    log_mini_debug("%u", 9507);  /* log_debug("starting master pop entry bit field generation");*/
    for (uint32_t master_pop_entry=0;
            master_pop_entry < population_table_length();
            master_pop_entry++) {
        ...
        log_mini_debug("%u%d%d%0x%d", 9508, master_pop_entry,  (uint32_t) key,  mask,  n_neurons);
        /* log_debug("pop entry %d, key = %d, mask = %0x, n_neurons = %d",master_pop_entry, (uint32_t) key, mask, n_neurons);*/
        bit_field_t bit_field = bit_field_alloc(n_neurons);
        if (bit_field == NULL) {
            log_mini_error("%u", 9509);  /* log_error("could not allocate dtcm for bit field");*/
            return false;
        }
        ...
        log_mini_debug("%u", 9510);  /* log_debug("searching neuron ids");*/
        for (uint32_t neuron_id=0; neuron_id < n_neurons; neuron_id++) {
            // update key with neuron id
            spike_t new_key = (spike_t) (key + neuron_id);
            log_mini_debug("%u%d%0x", 9511,  neuron_id,  new_key);  /* log_debug("new key for neurons %d is %0x", neuron_id, new_key);*/
            ...
            if (!bit_found) {
                if (population_table_get_first_address(
                    ...
                    log_mini_debug("%u%d", 9512,  neuron_id);  /* log_debug("%d", neuron_id);*/

                    // This is a direct row to process, so will have 1 target,
                    // so no need to go further
                    if (n_bytes_to_transfer == 0) {
                        log_mini_debug("%u", 9513);  /* log_debug("direct synapse");*/
                        bit_found = true;
                    } else {
                        // sdram read (faking dma transfer)
                        log_mini_debug("%u", 9514);  /* log_debug("dma read synapse");*/
                    ...
                    while (!bit_found && population_table_get_next_address(
                            &new_key, &row_address, &n_bytes_to_transfer)){
                        log_mini_debug("%u%d", 9515,  neuron_id);  /* log_debug("%d", neuron_id);*/

                        // This is a direct row to process, so will have 1
                        // target, so no need to go further
                        if (n_bytes_to_transfer == 0) {
                            log_mini_debug("%u", 9516);  /* log_debug("direct synapse");*/
                            bit_found = true;
                        } else {
                            // sdram read (faking dma transfer)
                            log_mini_debug("%u", 9517);  /* log_debug("dma read synapse");*/
                            bit_found = do_sdram_read_and_test(
                                    row_address, n_bytes_to_transfer);
                        }
                    }
            ...
            log_mini_debug("%u%d", 9518,  bit_found);  /* log_debug("bit_found %d", bit_found);*/
            if (bit_found) {
                bit_field_set(bit_field, neuron_id);
            }
        }

        log_mini_debug("%u", 9519);  /* log_debug("writing bitfield to sdram for core use");*/
        bit_field_base_address->filters[master_pop_entry].key = key;
        log_mini_debug("%u%d%d", 9520,  key,  master_pop_entry);  /* log_debug("putting master pop key %d in entry %d", key, master_pop_entry);*/
        bit_field_base_address->filters[master_pop_entry].n_atoms = n_neurons;
        log_mini_debug("%u%d%d", 9521,  n_neurons,  master_pop_entry);  /* log_debug("putting n_atom %d in entry %d", n_neurons, master_pop_entry);*/
        // write bitfield to sdram.
        log_mini_debug("%u%0x%d", 9522, &bit_field_words_location[position],  n_words);
        /* log_debug("writing to address %0x, %d words to write",&bit_field_words_location[position], n_words);*/
        ...
        log_mini_debug("%u", 9523);  /* log_debug("freeing the bitfield dtcm");*/
        sark_free(bit_field);
    }
    sort_by_redunancy();
    return true;
...
void c_main(void) {
    ...
    log_mini_info("%u", 9524);  /* log_info("starting the bit field expander");*/
    ...
    if (!initialise()) {
        log_mini_error("%u", 9525);  /* log_error("failed to init the master pop and synaptic matrix");*/
        fail_shut_down();
    }

    if (can_run) {
        log_mini_info("%u", 9526);  /* log_info("generating bit field");*/
        if (!generate_bit_field()) {
            log_mini_error("%u", 9527);  /* log_error("failed to generate bitfield");*/
            fail_shut_down();
        } else {
            success_shut_down();
            log_mini_info("%u", 9528);  /* log_info("successfully processed the bitfield");*/
        }
    }
}
