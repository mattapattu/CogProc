// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/robot_motor_control/robot_motor_control.c
...
static inline void do_motion(
    ...
    log_mini_debug("%u%s%d%s%d%d%u", 9402, direction,  direction_count,  opposite,  opposite_count,  delta, delta_threshold);
    /* log_debug("%s = %d, %s = %d, delta = %d, threshold = %u",direction, direction_count, opposite, opposite_count, delta,delta_threshold);*/


    if (delta >= delta_threshold) {
        log_mini_debug("%u%s", 9403,  direction);  /* log_debug("Moving %s", direction);*/
        last_speed[direction_index - 1] = speed;
        last_speed[opposite_index - 1] = 0;
        send_to_motor(direction_index, speed);
    } else if (delta <= -delta_threshold) {
        log_mini_debug("%u%s", 9404,  direction);  /* log_debug("Moving %s", direction);*/
        ...
        log_mini_debug("%u%s%s", 9405, direction,  opposite);
        /* log_debug("Motion is indeterminate in %s-%s direction",direction, opposite);*/
        last_speed[direction_index - 1] = 0;
        last_speed[opposite_index - 1] = 0;
...
static inline void do_update(
    ...
    if (delta > 0) {
        log_mini_debug("%u%s%d", 9406,  direction,  direction_speed);  /* log_debug("Resending %s = %d", direction, direction_speed);*/
        send_to_motor(direction_index, direction_speed);
    } else if (delta < 0) {
        log_mini_debug("%u%s%d", 9407,  opposite,  opposite_speed);  /* log_debug("Resending %s = %d", opposite, opposite_speed);*/
        send_to_motor(opposite_index, opposite_speed);
    } else {
        log_mini_debug("%u%s%s", 9408,  direction, opposite);
        /* log_debug("Resending No Motion in the %s-%s direction", direction,opposite);*/
        send_to_motor(direction_index, 0);
    }
}
...
static void timer_callback(uint unused0, uint unused1) {
    ...
    log_mini_debug("%u%d", 9409,  time);  /* log_debug("Timer tick %d", time);*/

    if ((infinite_run != TRUE) && (time == simulation_ticks)) {
        simulation_handle_pause_resume(NULL);
        log_mini_info("%u", 9410);  /* log_info("Simulation complete.\n");*/
    ...
    while (in_spikes_get_next_spike(&s)) {
        ...
        if (nid < N_COUNTERS) {
            ...
            log_mini_debug("%u%d", 9411,  nid);  /* log_debug("Received spike from unknown neuron %d", nid);*/
        }
    }
...
static void read_parameters(motor_control_config_t *config_region) {
    log_mini_info("%u%.8x", 9412,  config_region);  /* log_info("Reading parameters from 0x%.8x", config_region);*/
    ...
    log_mini_info("%u%d%d%d%d%d%d%d", 9413, key,  speed,  sample_time,  update_time,  delay_time,  delta_threshold, continue_if_not_different);
    /* log_info("Key = %d, speed = %d, sample_time = %d, update_time = %d, delay_time = %d, delta_threshold = %d, continue_if_not_different = %d",key, speed, sample_time, update_time, delay_time, delta_threshold,continue_if_not_different);*/
...
static void incoming_spike_callback(uint key, uint payload) {
    ...
    log_mini_debug("%u%x%d", 9414,  key,  time);  /* log_debug("Received spike %x at time %d\n", key, time);*/
...
static bool initialize(uint32_t *timer_period) {
    log_mini_info("%u", 9415);  /* log_info("initialise: started");*/
    ...
    log_mini_info("%u", 9416);  /* log_info("initialise: completed successfully");*/
...
void c_main(void) {
    ...
    if (!initialize(&timer_period)) {
        log_mini_error("%u", 9417);  /* log_error("Error in initialisation - exiting!");*/
        rt_error(RTE_SWERR);
    }
