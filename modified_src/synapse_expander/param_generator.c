// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/synapse_expander/param_generator.c
...
param_generator_t param_generator_init(uint32_t hash, address_t *in_region) {
    ...
    for (uint32_t i = 0; i < N_PARAM_GENERATORS; i++) {
        ...
        if (hash == type->hash) {
            ...
            if (generator == NULL) {
                log_mini_error("%u", 9356);  /* log_error("Could not create generator");*/
                return NULL;
            }
    ...
    log_mini_error("%u%u", 9357,  hash);  /* log_error("Param generator with hash %u not found", hash);*/
    return NULL;
}
