// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/synapse_expander/synapse_expander.c
...
static bool read_connection_builder_region(address_t *in_region,
    ...
    *in_region = region;
    ...
    log_mini_debug("%u%u%u", 9347, config.offset,  config.delayed_offset);
    /* log_debug("Synaptic matrix offset = %u, delayed offset = %u",config.offset, config.delayed_offset);*/
    log_mini_debug("%u%u%u", 9348, config.max_row_n_synapses,  config.max_delayed_row_n_synapses);
    /* log_debug("Max row synapses = %u, max delayed row synapses = %u",config.max_row_n_synapses, config.max_delayed_row_n_synapses);*/
    ...
    log_mini_debug("%u%08x%08x", 9349, synaptic_matrix,  delayed_synaptic_matrix);
    /* log_debug("Generating matrix at 0x%08x, delayed at 0x%08x",synaptic_matrix, delayed_synaptic_matrix);*/
    ...
    // If failed, log error
    if (!status) {
        log_mini_error("%u", 9350);  /* log_error("\tMatrix generation failed");*/
        return false;
    }
...
static bool run_synapse_expander(
    ...
    log_mini_info("%u%u%u%u", 9351, config.n_in_edges,  config.post_slice_count,  config.post_slice_start);
    /* log_info("Generating %u edges for %u atoms starting at %u",config.n_in_edges, config.post_slice_count, config.post_slice_start);*/
...
void c_main(void) {
    ...
    log_mini_info("%u", 9352);  /* log_info("Starting To Build Connectors");*/
    ...
    log_mini_info("%u%08x%08x", 9353, params_address,  syn_mtx_addr);
    /* log_info("\tReading SDRAM at 0x%08x, writing to matrix at 0x%08x",params_address, syn_mtx_addr);*/

    // Run the expander
    if (!run_synapse_expander(
            (address_t) params_address, (address_t) syn_mtx_addr)) {
        log_mini_info("%u", 9354);  /* log_info("!!!   Error reading SDRAM data   !!!");*/
        rt_error(RTE_ABORT);
    }

    log_mini_info("%u", 9355);  /* log_info("Finished On Machine Connectors!");*/
}
