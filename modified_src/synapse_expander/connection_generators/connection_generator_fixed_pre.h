// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/synapse_expander/connection_generators/connection_generator_fixed_pre.h
...
static void *connection_generator_fixed_pre_initialise(address_t *region) {
    ...
    *region = (void *) params_sdram;
    ...
    log_mini_debug("%u%u%u%u%u%u%u%u%u", 9395, obj->params.pre_lo,  obj->params.pre_hi,  obj->params.post_lo,  obj->params.post_hi, obj->params.allow_self_connections, obj->params.with_replacement,  obj->params.n_pre, obj->params.n_pre_neurons);
    /* log_debug("Fixed Total Number Connector,  pre_lo = %u, pre_hi = %u, post_lo = %u, post_hi = %u, allow self connections = %u, with replacement = %u, n_pre = %u, n pre neurons = %u",obj->params.pre_lo, obj->params.pre_hi, obj->params.post_lo, obj->params.post_hi,obj->params.allow_self_connections,obj->params.with_replacement, obj->params.n_pre,obj->params.n_pre_neurons);*/
...
uint32_t connection_generator_fixed_pre_generate(
    ...
    log_mini_debug("%u%u%u", 9396,  n_conns,  n_values);  /* log_debug("Generating %u from %u possible synapses", n_conns, n_values);*/
    ...
    if (pre_neuron_index == 0) {
        ...
        if (fixed_pre_globals.full_indices != NULL) {
            log_mini_error("%u", 9397);  /* log_error("Created out of order!");*/
            rt_error(RTE_SWERR);
        }
        ...
        if (array == NULL) {
            log_mini_warning("%u", 9398);  /* log_warning("Could not allocate in DTCM, trying SDRAM");*/
            array = sark_xalloc(sv->sdram_heap,
                    n_columns * n_conns * sizeof(uint16_t), 0, ALLOC_LOCK);
            fixed_pre_globals.in_sdram = true;
        }
        if (array == NULL) {
            log_mini_error("%u", 9399);  /* log_error("Could not allocate array for indices");*/
            rt_error(RTE_SWERR);
        }
