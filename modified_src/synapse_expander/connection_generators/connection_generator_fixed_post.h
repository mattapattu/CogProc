// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/synapse_expander/connection_generators/connection_generator_fixed_post.h
...
static void *connection_generator_fixed_post_initialise(address_t *region) {
    ...
    *region = (void *) params_sdram;
    ...
    log_mini_debug("%u%u%u%u%u%u%u%u%u", 9381, obj->params.pre_lo,  obj->params.pre_hi, obj->params.post_lo,  obj->params.post_hi, obj->params.allow_self_connections, obj->params.with_replacement,  obj->params.n_post, obj->params.n_post_neurons);
    /* log_debug("Fixed Number Post Connector, pre_lo = %u, pre_hi = %u, post_lo = %u, post_hi = %u, allow self connections = %u, with replacement = %u, n_post = %u, n post neurons = %u",obj->params.pre_lo, obj->params.pre_hi,obj->params.post_lo, obj->params.post_hi,obj->params.allow_self_connections,obj->params.with_replacement, obj->params.n_post,obj->params.n_post_neurons);*/
...
static uint32_t connection_generator_fixed_post_generate(
    ...
    log_mini_debug("%u%u%u", 9382,  n_conns,  n_values);  /* log_debug("Generating %u from %u possible synapses", n_conns, n_values);*/
...
//        		(j >= obj->params.post_lo) && (j <= obj->params.post_hi)) {
    ...
    for (unsigned int i = 0; i < count_indices; i++) {
     log_mini_info("%u%u%u", 9383,  i,  indices[i]);  /* log_info("Check: indices[%u] is %u", i, indices[i]);*/
    }
    log_mini_info("%u%u%u", 9384,  pre_neuron_index,  count_indices);  /* log_info("pre_neuron_index is %u count_indices is %u", pre_neuron_index, count_indices);*/
#endif
