// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/synapse_expander/connection_generators/connection_generator_fixed_total.h
...
static void *connection_generator_fixed_total_initialise(address_t *region) {
    ...
    *region = (void *) params_sdram;
    ...
    log_mini_debug("%u%u%u%u%u%u%u%u%u", 9392, obj->params.pre_lo,  obj->params.pre_hi, obj->params.post_lo,  obj->params.post_hi, obj->params.allow_self_connections, obj->params.with_replacement,  obj->params.n_connections, obj->params.n_potential_synapses);
    /* log_debug("Fixed Total Number Connector, pre_lo = %u, pre_hi = %u, post_lo = %u, post_hi = %u, allow self connections = %u, with replacement = %u, n connections = %u, n potential connections = %u",obj->params.pre_lo, obj->params.pre_hi,obj->params.post_lo, obj->params.post_hi,obj->params.allow_self_connections,obj->params.with_replacement, obj->params.n_connections,obj->params.n_potential_synapses);*/
...
static uint32_t connection_generator_fixed_total_generate(
    ...
    if (n_conns > max_row_length) {
        if (pre_neuron_index == obj->params.pre_hi) {
            log_mini_warning("%u%u", 9393, n_conns - max_row_length);
            /* log_warning("Could not create %u connections",n_conns - max_row_length);*/
        }
        n_conns = max_row_length;
    }
    log_mini_debug("%u%u%u", 9394, n_conns,  obj->params.n_connections);
    /* log_debug("Generating %u of %u synapses",n_conns, obj->params.n_connections);*/
