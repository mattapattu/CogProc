// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/synapse_expander/param_generators/param_generator_kernel.h
...
static void *param_generator_kernel_initialize(address_t *region) {
    ...
    *region += obj->params.kernelHeight * obj->params.kernelWidth;
    ...
    log_mini_debug("%u%u%u", 9360, obj->params.kernelWidth,  obj->params.kernelHeight);
    /* log_debug("Kernel param generator; kernelWidth, kernelHeight = %u,%u",obj->params.kernelWidth, obj->params.kernelHeight);*/
...
static void param_generator_kernel_generate(
    ...
    for (uint16_t i = 0; i < n_synapses; i++) {
        ...
        if ((0 <= k_r) && (k_r < params->kernelHeight) && (0 <= k_c)
            ...
            log_mini_error("%u%d%d", 9361,  k_r,  k_c);  /* log_error("Kernel coordinates off range (%d, %d)", k_r, k_c);*/
        }
    }
}
