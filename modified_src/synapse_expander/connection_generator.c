// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/synapse_expander/connection_generator.c
...
connection_generator_t connection_generator_init(
    ...
    for (uint32_t i = 0; i < N_CONNECTION_GENERATORS; i++) {
        ...
        if (hash == type->hash) {
            ...
            if (generator == NULL) {
                log_mini_error("%u", 9358);  /* log_error("Could not create generator");*/
                return NULL;
            }
    ...
    log_mini_error("%u%u", 9359,  hash);  /* log_error("Connection generator with hash %u not found", hash);*/
    return NULL;
}
