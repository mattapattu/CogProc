// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/synapse_expander/matrix_generators/matrix_generator_common.h
...
static struct delay_value get_delay(uint16_t delay_value, uint32_t max_stage) {
    ...
    if (delay < 1) {
        log_mini_debug("%u%u", 9368,  delay);  /* log_debug("Delay of %u is too small", delay);*/
        delay = 1;
    }
    ...
    if (stage >= max_stage) {
        log_mini_debug("%u%u", 9369,  delay);  /* log_debug("Delay of %u is too big", delay);*/
        stage = max_stage - 1;
