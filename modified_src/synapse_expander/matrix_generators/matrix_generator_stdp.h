// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/synapse_expander/matrix_generators/matrix_generator_stdp.h
...
void matrix_generator_stdp_write_row(
    ...
    for (uint32_t synapse = 0; synapse < n_synapses; synapse++) {
        ...
        if (pp_address[delay.stage] == NULL) {
            log_mini_error("%u%u", 9379,  delay.stage);  /* log_error("Delay stage %u has not been initialised", delay.stage);*/
            rt_error(RTE_SWERR);
        }

        // Check there is enough space
        if (space_half_words[delay.stage] < obj->n_half_words_per_pp_synapse) {
            log_mini_warning("%u%u%u%u", 9380, delay.stage,  space_half_words[delay.stage], obj->n_half_words_per_pp_synapse);
            /* log_warning("Row %u only has %u half words of %u free - not writing",delay.stage, space_half_words[delay.stage],obj->n_half_words_per_pp_synapse);*/
