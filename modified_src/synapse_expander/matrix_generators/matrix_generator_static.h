// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/synapse_expander/matrix_generators/matrix_generator_static.h
...
static void matrix_generator_static_write_row(
    ...
    log_mini_debug("%u%u", 9370,  max_stage);  /* log_debug("Max stage = %u", max_stage);*/
    ...
    log_mini_debug("%u%08x", 9371,  row_address[0]);  /* log_debug("row[0] = 0x%08x", row_address[0]);*/
    ...
    if (delayed_synaptic_matrix != NULL) {
        ...
        for (uint32_t i = 1; i < max_stage; i++) {
            ...
            log_mini_debug("%u%u%08x", 9372,  i,  row_address[i]);  /* log_debug("row[%u] = 0x%08x", i, row_address[i]);*/
        }
    } else {
        for (uint32_t i = 1; i < max_stage; i++) {
            row_address[i] = NULL;
            space[i] = 0;
            log_mini_debug("%u%u%08x", 9373,  i,  row_address[i]);  /* log_debug("row[%u] = 0x%08x", i, row_address[i]);*/
        }
    }
    ...
    for (uint32_t i = 0; i < max_stage; i++) {
        if (row_address[i] != NULL) {
            log_mini_debug("%u%08x%u", 9374, &row_address[i][STATIC_FIXED_FIXED_SIZE],  i);
            /* log_debug("Row size at 0x%08x for stage %u",&row_address[i][STATIC_FIXED_FIXED_SIZE], i);*/
        ...
        log_mini_debug("%u%u%08x", 9375,  i,  write_address[i]);  /* log_debug("write[%u] = 0x%08x", i, write_address[i]);*/
    }
    ...
    for (uint32_t synapse = 0; synapse < n_synapses; synapse++) {
        ...
        if (write_address[delay.stage] == NULL) {
            log_mini_error("%u%u%u%u%u", 9376,  delay.stage,  delays[synapse], delay.delay,  max_stage);
            /* log_error("Delay stage %u has not been initialised; raw delay = %u, delay = %u, max stage = %u", delay.stage, delays[synapse],delay.delay, max_stage);*/
        ...
        if (space[delay.stage] == 0) {
            log_mini_warning("%u%u", 9377, delay.stage);
            /* log_warning("Row for delay stage %u is full - word not added!",delay.stage);*/
            continue;
        }
        ...
        // Write the word
        log_mini_debug("%u%08x", 9378,  &write_address[delay.stage][0]);  /* log_debug("Writing word to 0x%08x", &write_address[delay.stage][0]);*/
