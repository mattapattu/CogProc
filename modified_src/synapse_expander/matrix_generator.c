// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/synapse_expander/matrix_generator.c
...
matrix_generator_t matrix_generator_init(uint32_t hash, address_t *in_region) {
    ...
    for (uint32_t i = 0; i < N_MATRIX_GENERATORS; i++) {
        ...
        if (hash == type->hash) {
            ...
            if (generator == NULL) {
                log_mini_error("%u", 9341);  /* log_error("Could not create generator");*/
                return NULL;
            }
    ...
    log_mini_error("%u%u", 9342,  hash);  /* log_error("Matrix generator with hash %u not found", hash);*/
    return NULL;
}
...
static inline uint16_t rescale_delay(accum delay, accum timestep_per_delay) {
    ...
    if (delay != delay_int) {
        log_mini_debug("%u%k%u", 9343,  delay,  delay_int);  /* log_debug("Rounded delay %k to %u", delay, delay_int);*/
    }
    return delay_int;
}
...
static inline uint16_t rescale_weight(accum weight, accum weight_scale) {
    ...
    if (weight != weight_int) {
        log_mini_debug("%u%k%u%k", 9344, weight,  weight_int,  weight_scale);
...
bool matrix_generator_generate(
    ...
    for (uint32_t i = 0; i < pre_slice_count; i++) {
        ...
        log_mini_debug("%u%u", 9345,  n_indices);  /* log_debug("Generated %u synapses", n_indices);*/
    ...
    log_mini_debug("%u%u", 9346,  n_connections);  /* log_debug("\t\tTotal synapses generated = %u. Done!", n_connections);*/
