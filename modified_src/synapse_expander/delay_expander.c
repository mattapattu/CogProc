// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/synapse_expander/delay_expander.c
...
static bool read_delay_builder_region(address_t *in_region,
    ...
    *in_region = region;
    ...
    for (uint32_t pre_neuron_index = pre_slice_start;
        ...
        log_mini_debug("%u%u", 9334,  n_indices);  /* log_debug("Generated %u synapses", n_indices);*/
        ...
        for (uint32_t i = 0; i < n_indices; i++) {
            ...
            if (delay != rounded_delay) {
                log_mini_debug("%u%k%u", 9335,  delay,  rounded_delay);  /* log_debug("Rounded delay %k to %u", delay, rounded_delay);*/
            }
...
static bool run_delay_expander(
    ...
    log_mini_info("%u%u%u%u", 9336, n_out_edges,  post_slice_count,  post_slice_start);
    /* log_info("Generating %u delay edges for %u atoms starting at %u",n_out_edges, post_slice_count, post_slice_start);*/
...
void c_main(void) {
    ...
    log_mini_info("%u", 9337);  /* log_info("Starting To Build Delays");*/
    ...
    log_mini_info("%u%08x%08x", 9338, delay_params_address,  params_address);
    /* log_info("\tReading SDRAM delay params at 0x%08x, expander params at 0x%08x",delay_params_address, params_address);*/


    // Run the expander
    if (!run_delay_expander(
            (address_t) delay_params_address, (address_t) params_address)) {
        log_mini_info("%u", 9339);  /* log_info("!!!   Error reading SDRAM data   !!!");*/
        rt_error(RTE_ABORT);
    }

    log_mini_info("%u", 9340);  /* log_info("Finished On-Machine Delays!");*/
}
