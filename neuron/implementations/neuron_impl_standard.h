...
static uint32_t neuron_impl_update_spiketime(uint32_t time, index_t neuron_index){
    ...
    }else if(neuron->lastProcessedSpikeTime - time  == deltaT){ // if lastProcessedSpikeTime is already been shifted once
        time = neuron->lastProcessedSpikeTime + deltaT;
        log_info("Updating neuron %u ext. inp to time = %u, lastProcessedSpikeTime = %u", neuron_index,time,neuron->lastProcessedSpikeTime);
    }
    return(time);
}
...
static bool  neuron_impl_neuron_update(index_t neuron_index,
    ...
    if(nextSpikeTime < INFINITY){
        ...
        while(ret == 1){
            ...
            ret = neuron_model_PDevs_sim(neuron, nextSpikeTime, key, neuron_index, nextInp,use_key);
        ...
        if(neuron_model_check_sim_continue(neuron)){
            return true;
        }else{
            log_info("End simulation = TRUE");
            return false;
        }
